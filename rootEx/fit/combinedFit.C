...
void combinedFit() {
   ...
   // functions to decompose signal and background
   //Double_t arr={20,12.,35.};
   UInt_t a=2;
   TF1 * fS1 = new TF1("fS","gaus",0,100);
   fS1->SetParameter(1,40);
   TF1 * fB1 = new TF1("fB","expo",0,100);
   ...
   hSB->SetFillColorAlpha(kGray,0.17);
   hSB->Draw();
   fSB->GetParameters(par);
   fB1->SetParameters(par);
   fS1->SetParameters(&par[2]);
   fS1->SetLineColor(kCyan);
   fB1->SetLineColor(kViolet);
   fB1->Draw("same");
   fS1->Draw("same");
   TLegend* leg=new TLegend(0.6,0.65,0.88,0.85);
   leg->SetTextFont(72);
   leg->SetTextSize(0.04);
   leg->AddEntry(hSB,"Data","lpe");
   leg->AddEntry(fSB,"Global Fit","l");
   leg->AddEntry(fB1,"Background Fit","l");
   leg->AddEntry(fS1,"Signal Fit","l");
   leg->Draw();
}
